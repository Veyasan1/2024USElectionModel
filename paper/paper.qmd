...
The dataset provided by fivethirtyeight contains over 15000 observations across 50 variables. Each observation is a poll conducted on the 2024 US Presidential Election. Pollsters are required by fivethirtyeight to disclose certain information about their poll, either publicly, or through an investigation. Of this information, a handful are key to the predictive model.
...
pollscore_summary <- just_harris_high_quality %>%
  summarize(
...
#### Plot data ####
just_harris_high_quality %>%
  ggplot(mapping=aes(x = state), y=count(n)) +
  geom_bar()


state_summary <- just_harris_high_quality %>%
  summarize(
    Min = min(count(state), na.rm = TRUE),
    Max = max(state, na.rm = TRUE)
  )
